...
**Assets & deploy.** Static project media (images, PDFs, board models, BOM files) is served from `public/portfolio/`. `public/CNAME` = `cameron-lewis.com` (a duplicate `CNAME` also sits at repo root) points the custom domain; `robots.txt` + `sitemap.xml` accompany it. Favicons are generated (`tools/build-favicon-ico.mjs` packs `icon-{16,32,48}.png` into `public/favicon.ico`). The résumé PDF is built outside this repo from LaTeX (`Career/Resume/src/resume.tex` in the Claude notes repo, compiled with `tectonic`); the served copy at `public/portfolio/assets/documents/resume/cameron-lewis-resume.pdf` is checked in by hand. The page-1 preview raster is what `ResumeViewer` actually renders (the viewer no longer uses PDF.js); regenerate it with `pnpm build:resume-preview <path-to-pdf>`, which refuses to run on a multi-page PDF.
