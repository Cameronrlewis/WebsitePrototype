...
// @ts-expect-error -- plain .mjs build tool with no type declarations.
import { encodeBoard } from "../tools/build-board-geometry-bin.mjs";
...
describe("shipped .pcbgeo binaries stay in sync with the bundle", () => {
  ...
  it.each(["power", "control", "brick"] as const)("%s binary matches its bundle block byte for byte", (name) => {
    // Encoding is deterministic, so re-encoding the bundle must reproduce the
    // shipped payload exactly: any drift in vertices, winding or offsets fails.
    const shipped = gunzipSync(readFileSync(path.join(geometryDir, `${name}.pcbgeo`)));
    expect(shipped.equals(encodeBoard(boards.get(name)!).buffer)).toBe(true);
  });
});
