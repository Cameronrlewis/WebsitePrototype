...
Create `public/portfolio/assets/viewers/tours/brick.tour.json`, substituting the four sets of numbers recorded in Step 2 for the `x`, `y` and `span` placeholders. Write the `label` and `blurb` from the part's actual identity in the IBOM and the Brick Buck Board copy in `src/app/src/app/data/portfolio.ts` (slug `brick-buck-board`), matching the register of `control.tour.json`: one short noun phrase for the label, and two sentences at most for the blurb, the first naming the part and the second saying what it does on this board. **No em-dashes.**

The shape, with the `id` values fixed because Step 5's test pins one of them:
  ...
  "stops": [
    {
      ...
      "blurb": "A Mornsun DC-DC module. It replaces the custom 48V to 12V stage so the backup board did not have to re-solve the high voltage conversion.",
      "x": 0,
      "y": 0,
      "span": 59.9
    },
    {
      ...
      "blurb": "The on-board switching regulator. It is the one power stage kept custom, sized around the low voltage subsystem loads.",
      "x": 0,
      "y": 0,
      "span": 0
    },
    {
      ...
      "blurb": "Per-rail protection on the output side. A fault in one subsystem trips its own device without pulling down the rest of the vehicle.",
      "x": 0,
      "y": 0,
      "span": 19.85
    },
...
This is also the step that confirms the centre-versus-corner decision recorded above. If a stop looks systematically offset by half the part, that is the corner convention leaking in and the numbers need `size / 2` added.
...
Run: `npx pnpm@10.17.1 exec playwright test e2e/board-spin.spec.ts -g "brick board has a tour"`
